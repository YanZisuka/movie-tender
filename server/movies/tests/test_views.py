...
class MovieViewSetTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.keywords = [
            KeywordFactory.create(keyword="anime"),
            KeywordFactory.create(keyword="superhero"),
        ]
    ...
    def test_서베이를_설정할수있다(self):
        data = json.dumps({"survey": ["anime", "superhero"]})
        res = self.client.put(
            reverse("movies:movies"),
            data=data,
        ...
        self.assertEqual(res.status_code, 200)
        self.assertEqual(
            get_user_model().objects.get(pk=self.user.id).survey, ["anime", "superhero"]
        )

    def test_영화상세정보를_볼수있다(self):
        res = self.client.get(
            reverse("movies:movie", args=[self.movies[0].id]), **self.header
        )
    ...
    def test_평점을_설정할수있다(self):
        res = self.client.post(
            reverse("movies:movie", args=[self.movie.id]),
            data={"rating": 3.5},
            **self.header,
        )
    ...
    def test_평점을_불러올수있다(self):
        res = self.client.get(
            reverse("movies:get_rating", args=[self.movies[0].id, self.user.username]),
            **self.header,
        )
        self.assertEqual(res.status_code, 200)
        ...
        res = self.client.post(
            reverse("movies:movie", args=[self.movies[0].id]),
            data={"rating": 4},
            **self.header,
        )
        res = self.client.get(
    ...
    def test_키워드에_맞는_영화를_뽑을수있다(self):
        ...
        res = self.client.get(
            reverse("movies:get_movies_with_keywords", args=[2]), **self.header
        )
