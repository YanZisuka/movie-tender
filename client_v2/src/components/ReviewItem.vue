    ...
    <div class="text-start mx-5" style="width: 100%">
      <div class="d-flex justify-content-between">
        ...
        <span v-if="review.user.id === currentUser.pk && !isEditing">
          <button v-if="false" @click="switchIsEditing" class="btn btn-sm">
            <i class="fa-solid fa-pen"></i>
          </button>
          <button @click="onDelete(review.id)" class="btn">
      ...
      <div class="d-flex">
        <div @click="likeReview(review.id)" class="btn-like me-3">
          <i
            :class="isLike ? 'active' : 'inactive'"
            class="me-2 fa-solid fa-heart"
          ></i>
          <span>{{ likeCount }}</span>
        </div>
