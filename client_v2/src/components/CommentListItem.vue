<template>
  <li
    :class="isMaxIdx ? '' : 'mb-4'"
    class="comment-list d-flex justify-content-between"
  >
    <div>
      <router-link
        class="fw-bold"
      ...
      <input
        v-if="currentUser.pk === comment.user.id && isEditing"
        type="text"
    ...
    <div>
      <div v-if="currentUser.pk === comment.user.id && !isEditing">
        <button class="btn btn-sm" @click="switchIsEditing">
          <i class="fa-solid fa-pen"></i>
        </button>
        <button class="btn btn-sm" @click="onDelete">
